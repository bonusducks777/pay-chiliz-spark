...
import { CreditCard, Loader2, CheckCircle, AlertTriangle, XCircle, Wallet } from 'lucide-react'
import { toast } from 'sonner'

export const StellarUserPanel = () => {
  const { networkConfig } = useNetwork()
  const stellarWallet = useStellarWallet()
  const [isPayingTx, setIsPayingTx] = useState(false)
  ...
  const stellarClient = networkConfig.contractAddress && networkConfig.rpcUrl
    ? new StellarContractClient(networkConfig.contractAddress, networkConfig.rpcUrl)
    : null

  // Fetch active transaction and payment status
  useEffect(() => {
    if (stellarClient && stellarWallet.isConnected) {
      const fetchData = async () => {
        try {
          const [activeTx, payStatus] = await Promise.all([
            stellarClient.getActiveTransaction(),
            stellarClient.getPaymentStatus()
          ])
  ...
  }, [stellarClient, stellarWallet.isConnected])

  const handlePayActiveTransaction = async () => {
    if (!stellarClient || !stellarWallet.isConnected) {
    ...
    if (!activeTransaction) {
      toast.error('No active transaction to pay')
      return
    }

    setIsPayingTx(true)
    try {
      await stellarClient.payActiveTransaction()
      toast.success('Payment submitted successfully!')
      
      // Refresh transaction data after payment
      setTimeout(async () => {
        const [activeTx, payStatus] = await Promise.all([
          stellarClient.getActiveTransaction(),
          stellarClient.getPaymentStatus()

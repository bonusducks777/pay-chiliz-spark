...
export const TronAdminPanel = () => {
  ...
  const [formData, setFormData] = useState({
    amount: '',
    description: '',
    requestedTokenContract: '', // Empty string for native TRX
  });

  // Merchant info state (like Chiliz version)
  const [merchantInfo, setMerchantInfo] = useState({
    name: '',
    location: ''
  });

  // Itemized items state (like Chiliz version)
  ...
  useEffect(() => {
    if (itemizedItems.length > 0) {
      ...
      setFormData(prev => ({ ...prev, amount: total ? total.toFixed(6) : '' }));
    } else {
  ...
  const handleCreateTransaction = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isConnected || !isOwner || !formData.amount) return;
  ...
  const handleWithdraw = async () => {
    if (!isConnected || !isOwner || !withdrawAddress) {
    ...
    try {
      setIsLoading(true);
      setError(null);
      // Use empty string for native TRX withdrawal
        ...
        <CardContent>
          <div className="text-center py-8">
            <p className="text-muted-foreground mb-4">
              Connect your TronLink wallet to access admin functions
            </p>
            <TronConnectButton />
          </div>
        </CardContent>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-[1fr_80px_100px_80px] gap-2 items-end">
                  ...
                  <div>
                    <Label htmlFor="itemValue">Value (TRX)</Label>
                    <Input
                      id="itemValue"
                      value={newItem.value}
                    ...
                    {itemizedItems.map((item, index) => (
                      <div key={index} className="flex justify-between items-center bg-muted/30 rounded px-2 py-1 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-mono">{item.value} TRX</span>
